...
int main()
{
    ...
    if (addition_or_removal == 'B') {
        ...
        printf("Quantidade do produto: %i e Tipo do Produto: %s", product_quantity, ultima_mensagem_recebida);
    ...
    payload.quantity = product_quantity;
    payload.operation = addition_or_removal;

    strncpy(payload.product_name, ultima_mensagem_recebida, sizeof(payload.product_name) - 1);

    payload.product_name[sizeof(payload.product_name) - 1] = '\0';

    // Envia o pacote de dados para o escravo
    int bytes_sent = i2c_write_blocking(I2C_PORT, SLAVE_ADDR, (uint8_t*)&payload, sizeof(I2C_Payload), false);
