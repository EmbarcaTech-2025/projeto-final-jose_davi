// bmp280.c
// Implementation of BMP280 sensor driver
// This file contains the functions to initialize the sensor, read raw data,
...
void bmp280_read_raw(int32_t *temp, int32_t *pressure) {
  ...
  i2c_read_blocking(I2C_PORT, ADDR, buf, 6, false); // false - finished with bus
...
void bmp280_get_calib_params(struct bmp280_calib_param *params) {
  ...
  i2c_read_blocking(I2C_PORT, ADDR, buf, NUM_CALIB_PARAMS,
...
void get_temp_pres(float *temp, float *pres) {
  bmp280_read_raw(&raw_temp, &raw_pres);
  int32_t temperature = bmp280_convert_temp(raw_temp, &params);
  int32_t pressure = bmp280_convert_pressure(raw_pres, raw_temp, &params);
  *temp = temperature / 100.f;
  *pres = pressure / 1000.f;
}
