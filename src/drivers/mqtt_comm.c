...
// Base:
// https://github.com/BitDogLab/BitDogLab-C/blob/main/wifi_button_and_led/lwipopts.h
...
#define MAX_MQTT_TOPIC_SIZE                                                    \
  128 // Tamanho máximo (em bytes) para armazenar o nome do tópico recebido
#define MAX_MQTT_PAYLOAD_SIZE                                                  \
  512 // Tamanho máximo (em bytes) para armazenar a mensagem recebida
static char
    incoming_topic_buf[MAX_MQTT_TOPIC_SIZE]; // Buffer para o nome do tópico da
                                             // mensagem recebida
static char
    incoming_payload_buf[MAX_MQTT_PAYLOAD_SIZE]; // Buffer para o conteúdo da
                                                 // mensagem recebida (payload)
static uint32_t incoming_payload_len;            // Tamanho da mensagem recebida
static uint8_t new_message_received =
    0; // Flag para indicar se uma nova mensagem foi recebida por completo

/* Callback de início de recepção de publicação MQTT
 * Chamado quando o cabeçalho de uma nova publicação é recebido (antes dos
 * dados).
 *
 * Parâmetros:
...
static void mqtt_incoming_publish_cb(void *arg, const char *topic,
  ...
  // printf("MQTT RX Publish: Tópico '%s', Tamanho Total do Payload: %lu
  // bytes\n", topic, tot_len);
  ...
  if (tot_len >= MAX_MQTT_PAYLOAD_SIZE) {
    // printf("MQTT RX Publish: Payload muito grande (%lu bytes vs %d max).
 ...
 *   - flags: flags que indicam se o fragmento é o último da mensagem
 * (MQTT_DATA_FLAG_LAST)
 */
static void mqtt_incoming_data_cb(void *arg, const u8_t *data, u16_t len,
  ...
  // Verifica se ao adicionar o novo fragmento haverá estouro do buffer de
  // payload
  if (incoming_payload_len + len > MAX_MQTT_PAYLOAD_SIZE - 1) {
    // printf("MQTT RX Data: Buffer de payload excedido ao concatenar.
    // Descartando fragmento.\n");
    return;
  }
  ...
  incoming_payload_buf[incoming_payload_len] =
      '\0'; // Garante terminação da string
...
void mqtt_comm_publish(const char *topic, const uint8_t *data, size_t len) {
  ...
  err_t status = mqtt_publish(client, // Instância do cliente
...
void mqtt_comm_subscribe(const char *topic) {
  ...
  err_t status = mqtt_subscribe(client, // Instância do cliente
